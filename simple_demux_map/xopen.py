...
class PipedXzReader(object):
    def __init__(self, path):
        self.process = Popen(['xz', '-cd', path], stdout=PIPE, stderr=PIPE)
        self.closed = False
        # Give xz a little bit of time to report any errors (such as
        # a non-existing file)
        time.sleep(0.01)
        self._raise_if_error()
